...
metadata = read.csv("data/artistic_trial/Masterfile_groups-MSc-project.csv", sep = ";")
...
meth <- readRDS("data/artistic_trial/df_meth.rds")
print("df meth has this many rows:")
print(nrow(meth))

df_meth <- methylKit::getData(meth)

myDiff <- readRDS("data/artistic_trial/calculateDiffMeth_object.rds")
print("myDiff has this many rows:")
print(nrow(myDiff))

df_beta_vals <- read.table("data/artistic_trial/artistic_study_betas_b4_EDMR_50threshold.txt", header=T, sep=";")
...
df_bed_file <- as.data.frame(read.table("data/hg19-blacklist.v2.bed",header = FALSE, sep="\t",stringsAsFactors=FALSE, quote=""))
...
# filter myDiff for q-value of 0.05 and methylation Difference of 10 
